...
Each concrete agent implements the `Agent` interface in `types.ts` (`name`, async `run(prompt, cwd, options)` returning `{ output, usage }`, optional `close()`). They share two responsibilities: stream stdout, extract a structured `AgentOutput` (`success`, `summary`, `key_changes_made`, `key_learnings`, commit-message fields when configured, `next_iteration_tier` when future tier-selection phases enable it, plus `should_fully_stop` only when `--stop-when` is active) that matches the schema built by `buildAgentOutputSchema(...)`, and accumulate `TokenUsage`. `factory.ts` now exposes `createAgentProvider(...)`, which freezes legacy runs to the single `default` tier and lazily creates per-tier agents when `runInfo.tieredModels` is active.
